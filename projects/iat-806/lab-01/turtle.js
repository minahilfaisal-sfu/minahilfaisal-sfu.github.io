...
//
// You don't need to understand this file to use it. Read turtle-sketch.js first.
// We come back to how classes like this are written in Week 4.
//
// This version takes the p5 instance ("p") as its first argument, so two
// separate sketches can each run their own KardunTurtle on the same page
// without their global setup()/draw() functions colliding.

class KardunTurtle {
  // p     — the p5 instance this turtle belongs to
  ...
  _paintIcon(target, x, y) {
    ...
    if (this.iconBuffer) {
      p.image(this.iconBuffer, x, y, this.size, this.size);
    } else {
      p.noStroke();
      p.fill(this.color);
